...
- **`PWM_ON_TIME_S`（実装済）**: 自動消灯タイマは平均点灯時間を下げて消費を削減。スリープ実装の
  有無に関わらず効く「今すぐ使えるレバー」。
- **深い低電力（ロードマップ・未実装）**: Standby＋AWU 周期起床（エンコーダは EXTI、スリープ中は
  プルアップをアナログ入力化してリーク停止）を実装した場合、単セル **CR2032(~220mAh)** で
  1kΩ・100ms/5s なら **約6.5ヶ月** と見積もり。**未実装（48MHz 常時稼働）ではこれより大幅に短い**。
