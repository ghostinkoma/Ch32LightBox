...
## 修正 (2026-09-20): 設定エディタでピン割当が編集できなかった

症状: `LightBoxMenu.jar` → 設定エディタで、肝心のピンのドロップダウンが割り当てられない。
原因は2つ、いずれも `ConfigEditor.java` で修正:

1. **ピンが「上級者モード」でロックされていた**（`PWM_PIN`/`ENC_*_PIN` が `.advanced()` 指定で、
   チェックを入れるまで無効=グレーアウト）。ピン割当は本ツールの主機能なので、トグルを
   **既定ON** に変更（ラベルを「ピン等の固定項目を編集する（オフでロック）」に）。オフでロック可。
2. **読み込み時の値クロバー**: 新設した電池目安のライブ再計算リスナが `loadWidgets()` 中に発火し、
   `applyWidgets()` が未ロードの既定ウィジェット値で `config` を上書き → ピン等が正しく読めなかった。
   `loading` ガードを追加し、ロード中はリスナ由来の適用/再計算を抑止。

（補足: `Settings.projectDir()` は jar 位置から上位へ `source/config.h` を自動検出するため、
`dist/` から起動してもセットアップ無しで config.h を見つける。）
